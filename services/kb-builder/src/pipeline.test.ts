...
describe('Pipeline', () => {
  ...
  describe('runPipeline', () => {
    ...
    it('should handle quality filter failure', async () => {
      vi.mocked(runQualityFilter).mockRejectedValue(new Error('MongoDB timeout'));
    ...
    it('should handle summarization failure', async () => {
      ...
      vi.mocked(getArticlesByStatus).mockResolvedValue(Array(5).fill({ id: 'a1' }));
      vi.mocked(runSummarization).mockRejectedValue(new Error('OpenAI API down'));
