...
async function fetchPage(url: string): Promise<string> {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'MitoAIJourney-KBBuilder/0.1 (internal knowledge base builder)',
 ...
 */
function discoverLinks(html: string, baseUrl: string): { links: string[]; title: string } {
...
function extractContent(
  ...
  const text = (articleEl.length > 0 ? articleEl : $('body')).text().replace(/\s+/g, ' ').trim();
    ...
    for (const link of linksToProcess) {
      try {
        ...
        if (content.wordCount < 50) {
          currentProgress.totalSkipped++;
          currentProgress.totalProcessed++;
          log('debug', `Skip (too short: ${content.wordCount} words): ${link}`, {
            url: link,
