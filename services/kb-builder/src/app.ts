...
import { addSSEClient, clearLogBuffer, getLogBuffer, log } from './log-stream.js';
...
app.post('/ingest', async (_req, res) => {
  ...
  void (async () => {
    ...
    log('info', 'Crawl finished — starting pipeline (quality → summarize → Pinecone RAG ingest)');
    await runPipeline();
  })();
...
app.get('/summarization-stats', async (_req, res) => {
  try {
    const [articles, summaries] = await Promise.all([getAllArticles(), getAllSummaries()]);
    ...
    const eligibleStatuses = new Set(['quality_passed', 'summarized', 'ingested']);
    const eligibleArticles = articles.filter((a) => eligibleStatuses.has(a.status));
    ...
    const percent = totalEligible > 0 ? Math.round((totalSummarized / totalEligible) * 100) : 0;

    // Token and cost stats
    const totalTokens = summaries.reduce((sum, s) => sum + s.tokensUsed, 0);
    const totalPromptTokens = summaries.reduce((sum, s) => sum + (s.promptTokens || 0), 0);
    const totalCompletionTokens = summaries.reduce((sum, s) => sum + (s.completionTokens || 0), 0);

    // gpt-5-nano pricing: $0.10/M input, $0.40/M output
    const inputCostPerM = 0.1;
    const outputCostPerM = 0.4;
    const totalCost =
      (totalPromptTokens * inputCostPerM + totalCompletionTokens * outputCostPerM) / 1_000_000;
    const costPerArticle = totalSummarized > 0 ? totalCost / totalSummarized : 0;
...
app.get('/rag/stats', async (_req, res) => {
  try {
    ...
    log('error', `RAG stats failed: ${err instanceof Error ? err.message : String(err)}`);
    res.status(500).json({
      error: {
