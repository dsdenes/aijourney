...
vi.mock("openai", () => {
	const mockCreate = vi.fn().mockResolvedValue({
		choices: [{ message: { content: '{"steps":[{"title":"Step 1","description":"Learn basics","task":"Do X","expectedOutput":"Y","estimatedMinutes":30,"tags":["tools"],"toolsRequired":["chatgpt"]}]}' } }],
					...
					data: responses.summaries ?? [
						{
							...
							content: {
								...
								difficulty: "beginner",
								roleRelevance: [
									{ role: "engineering", relevanceScore: 0.8 },
								],
							},
						},
					],
...
describe("Summarization Worker", () => {
	...
	it("should handle pipeline trigger failure", async () => {
		...
		await expect(handleSummarizationJob(job)).rejects.toThrow(
			"KB Builder pipeline returned 500",
		);
	});
});
...
describe("Personalization Worker", () => {
	...
	it("should log processing details", async () => {
		...
		expect(consoleSpy).toHaveBeenCalledWith(
			expect.stringContaining("user=user-2"),
		);
...
describe("KB Chat Worker", () => {
	...
	it("should log processing with user context", async () => {
		...
		expect(consoleSpy).toHaveBeenCalledWith(
			expect.stringContaining("run=run-3"),
		);
		expect(consoleSpy).toHaveBeenCalledWith(
			expect.stringContaining("user=user-3"),
		);
