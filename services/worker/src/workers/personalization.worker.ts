 ...
 */
export async function handlePersonalizationJob(job: Job): Promise<Record<string, unknown>> {
  const { runRequestId, userId, journeyId } = job.data;
  const startedAt = Date.now();

  await job.log(`start run=${runRequestId} user=${userId} journey=${journeyId}`);
  console.log(
  ...
  try {
    ...
    const roleSummaries = summaries
      .map((s) => {
        const relevance = s.content.roleRelevance?.find((r) => r.role.toLowerCase() === userRole);
    ...
    await job.log(`Generated ${steps.length} personalized steps (${tokensUsed} tokens)`);
