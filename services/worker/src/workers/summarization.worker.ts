 ...
 */
export async function handleSummarizationJob(
	job: Job,
	...
	console.log(
		`[summarization] Processing run=${runRequestId} article=${articleId || "all"}`,
	);
	...
	try {
		...
		if (!res.ok) {
			...
			throw new Error(
				`KB Builder pipeline returned ${res.status}: ${text.slice(0, 200)}`,
			);
		}
		...
		while (attempts < maxAttempts) {
			...
			try {
				const progressRes = await fetch(
					`${KB_BUILDER_URL}/pipeline/progress`,
				);
				if (progressRes.ok) {
					const progress = (await progressRes.json()) as Record<
						string,
						unknown
					>;
					...
					await job.updateProgress(
						Math.min(50 + Math.floor((attempts / maxAttempts) * 45), 95),
					);
					...
					if (status === "failed") {
						throw new Error(
							`Pipeline failed: ${(progress.error as string) || "unknown error"}`,
						);
					}
				}
			} catch (pollErr) {
				if (
					...
					pollErr.message.startsWith("Pipeline failed")
				) {
					throw pollErr;
				}
