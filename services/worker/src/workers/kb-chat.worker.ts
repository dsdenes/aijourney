 ...
 */
export async function handleKbChatJob(job: Job): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const estimatedTokens =
      Math.ceil((SYSTEM_PROMPT.length + contextMsg.length + (query as string).length) / 4) + 1500;
