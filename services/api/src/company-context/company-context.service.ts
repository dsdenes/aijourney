...
export class CompanyContextService {
	...
	async uploadDocument(
		...
		this.runExtraction(tenantId, docId).catch((err) => {
			this.logger.error(
				`Background extraction failed for ${docId}: ${(err as Error).message}`,
			);
		});
	...
	async reExtract(tenantId: string, docId: string): Promise<CompanyDocument> {
		...
		this.runExtraction(tenantId, docId).catch((err) => {
			this.logger.error(
				`Re-extraction failed for ${docId}: ${(err as Error).message}`,
			);
		});
	...
	private async runExtraction(tenantId: string, docId: string): Promise<void> {
		try {
			...
			// Parse text
			const text = await this.extraction.parseDocumentText(
				buffer,
				doc.mimeType,
			);
			...
			this.logger.log(
				`Extraction complete for doc ${docId}: ${facts.length} facts`,
			);
		} catch (err) {
