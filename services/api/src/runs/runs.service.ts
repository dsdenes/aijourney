import type { CreateRunRequestInput, RunRequest, RunStatus } from '@aijourney/shared';
import {
  DEFAULT_BUDGETS,
  generateId,
  isValidTransition,
...
export class RunsService {
  ...
  async create(userId: string, input: CreateRunRequestInput): Promise<RunRequest> {
    const now = nowISO();
    ...
    this.logger.log(`Run ${run.id} created (${run.purpose}) status=${run.status}`);
    return run;
  }
  ...
  async transition(
    ...
    if (!isValidTransition(RUN_TRANSITIONS, run.status, toStatus)) {
      throw new BadRequestException(`Cannot transition from ${run.status} to ${toStatus}`);
    }
  ...
  async cancel(id: string, cancelledBy: string): Promise<RunRequest> {
    const run = await this.getById(id);
    const toStatus: RunStatus = run.status === 'RUNNING' ? 'CANCEL_REQUESTED' : 'CANCELLED';
