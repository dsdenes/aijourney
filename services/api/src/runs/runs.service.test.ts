...
describe('RunsService', () => {
  ...
  describe('transition', () => {
    it('should allow valid transitions (PENDING -> APPROVED)', async () => {
      ...
      repo.getById.mockResolvedValueOnce(run).mockResolvedValueOnce({ ...run, status: 'APPROVED' });
    ...
    it('should reject invalid transitions (PENDING -> RUNNING)', async () => {
      ...
      await expect(service.transition('r1', 'RUNNING')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('reject', () => {
    it('should transition PENDING to REJECTED', async () => {
      const run = { id: 'r1', status: 'PENDING' };
      repo.getById.mockResolvedValueOnce(run).mockResolvedValueOnce({ ...run, status: 'REJECTED' });
      repo.updateStatus.mockResolvedValue(undefined);
