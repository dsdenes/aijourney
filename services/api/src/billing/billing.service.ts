...
import { BadRequestException, Inject, Injectable, Logger, RawBodyRequest } from '@nestjs/common';
...
export class BillingService {
  ...
  private async handleCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
    ...
    if (session.metadata?.['type'] === 'llm_pack') {
      ...
      if (tenant) {
        await this.tenantsRepo.updateRaw(tenantId, {
          'quotas.additionalLlmCalls': tenant.quotas.additionalLlmCalls + additionalCalls,
        });
    ...
    if (plan) {
      ...
      const subscriptionId = (session as unknown as Record<string, unknown>).subscription as
        | string
        | undefined;
      if (subscriptionId) {
