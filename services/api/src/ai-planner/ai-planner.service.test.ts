...
describe('AiPlannerService', () => {
  ...
  describe('generateQuestions', () => {
    it('should return 6 questions for round 1', async () => {
      ...
      const result = await service.generateQuestions('Build a customer support chatbot', 1, []);
    ...
    it('should pass previous answers in context for rounds 2 and 3', async () => {
      ...
      const result = await service.generateQuestions('Build a chatbot', 2, previousAnswers);
    ...
    it('should throw on empty response', async () => {
      ...
      await expect(service.generateQuestions('Build a chatbot', 1, [])).rejects.toThrow(
        'Empty response',
      );
    });
  ...
  describe('generateStrategy', () => {
    ...
    it('should fail loudly when OPENAI_API_KEY is missing', async () => {
      ...
      await expect(
        service.generateStrategy('Simple task', [{ id: 1, question: 'Quick?', answer: true }]),
