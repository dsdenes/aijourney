...
export class MemoryService implements OnModuleInit, OnModuleDestroy {
	...
	async onModuleInit() {
		...
		this.worker.on("failed", (job, err) => {
			this.logger.warn(
				`Memory extraction job ${job?.id} failed: ${err.message}`,
			);
		});

		this.logger.log(
			`Memory extraction queue initialized (rate limit: ${RATE_LIMIT_MAX}/min)`,
		);
	}
	...
	async enqueueExtraction(
		...
		this.logger.debug(
			`Enqueued memory extraction for user ${userId} from ${source}`,
		);
	}
	...
	async getStats(): Promise<MemoryStats> {
		const [totalFacts, factsByCategory, factsBySource, recentExtractions] =
			await Promise.all([
