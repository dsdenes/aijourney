import type { MemoryCategory, MemoryExtraction, MemoryFact, MemorySource } from '@aijourney/shared';
...
export class MemoryRepository {
  ...
  constructor(@Inject(MONGODB_DB) db: Db) {
    ...
    this.extractions = db.collection<MemoryExtractionDoc>('memory_extractions');
  }
  ...
  async getAllFactsByUser(userId: string): Promise<MemoryFact[]> {
    const docs = await this.facts.find({ userId }).sort({ createdAt: -1 }).toArray();
    return docs.map(fromFactDoc);
  }

  async getFactsByCategory(userId: string, category: MemoryCategory): Promise<MemoryFact[]> {
    const docs = await this.facts
  ...
  async supersedeFact(factId: string, newFactId: string): Promise<void> {
    await this.facts.updateOne({ _id: factId }, { $set: { supersededBy: newFactId } });
  }
  ...
  async countFactsByCategory(userId: string): Promise<Record<MemoryCategory, number>> {
    const pipeline = [
  ...
  async tenantCategoryCounts(tenantId: string): Promise<Record<MemoryCategory, number>> {
    const pipeline = [
  ...
  async tenantSourceCounts(tenantId: string): Promise<Record<MemorySource, number>> {
    const pipeline = [
      { $match: { tenantId, supersededBy: { $exists: false } } },
      { $group: { _id: '$source', count: { $sum: 1 } } },
    ];
  ...
  async getRecentExtractions(limit = 20): Promise<MemoryExtraction[]> {
    const docs = await this.extractions.find().sort({ processedAt: -1 }).limit(limit).toArray();
    return docs.map(fromExtractionDoc);
  }

  async getExtractionsByUser(userId: string, limit = 20): Promise<MemoryExtraction[]> {
