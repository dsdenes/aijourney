...
export class ArticleRecsService implements OnModuleInit, OnModuleDestroy {
	...
	async onModuleInit() {
		...
		this.worker.on("failed", (job, err) => {
			this.logger.error(
				`Recommendation batch job ${job?.id} failed: ${err.message}`,
			);
		});
		...
		this.logger.log(
			`Article recommendations initialized. Schedule: ${SCHEDULE_CRON_PATTERN}`,
		);
	}
	...
	async getAdminStats(tenantId?: string): Promise<RecAdminStats> {
		...
		const readRate =
			total > 0 ? Number(((counts.read / total) * 100).toFixed(1)) : 0;
	...
	private async processBatch(
		...
		try {
			...
			this.logger.log(
				`Batch ${batchId}: ${activeJobTitles.length} active job titles`,
			);
			...
			for (const jobTitle of activeJobTitles) {
				...
				const candidates =
					allCandidates.find((c) => c.jobTitle === jobTitle)?.articles ??
					[];
				...
				for (const user of users) {
					try {
						const rec = await this.selectArticleForUser(
							user,
							candidates,
							jobTitle,
							batchId,
	 ...
	 */
	private async getUsersByJobTitle(
		tenantId?: string,
	...
	private async fetchCandidatesForJobTitles(
		...
		for (const jobTitle of jobTitles) {
			...
			try {
				...
				if (!ragResponse.ok) {
					throw new Error(
						`RAG query failed: ${ragResponse.status} ${await ragResponse.text()}`,
					);
				}
				...
				const currentBatch =
					await this.recsRepo.getBatchById(batchId);
				if (currentBatch) {
					await this.recsRepo.updateBatch(batchId, {
						agentRunIds: [
							...currentBatch.agentRunIds,
							agentRun.id,
						],
					});
				}
				...
				this.logger.log(
					`Fetched ${articles.length} candidates for "${jobTitle}"`,
				);
			} catch (err) {
				const message =
					err instanceof Error ? err.message : String(err);
				...
				this.logger.error(
					`Failed to fetch candidates for "${jobTitle}": ${message}`,
				);
	...
	private async buildRagQueryForJobTitle(jobTitle: string): Promise<string> {
		...
		return (
			...
			`AI tools and workflows for ${jobTitle}`
		);
	}
...
${articleList}`,
			...
			const idx = Math.max(
				0,
				Math.min(candidates.length - 1, (parsed.selectedIndex || 1) - 1),
			);
