...
export class WorkersController {
  ...
  async updateKbBuilderSource(@Param('id') id: string, @Body() body: Record<string, unknown>) {
  ...
  async triggerJob(@Param('slug') slug: string, @Body() body: Record<string, unknown>) {
    if (slug === 'kb-builder') {
  ...
  async cleanQueue(@Param('slug') slug: string, @Param('status') status: 'completed' | 'failed') {
