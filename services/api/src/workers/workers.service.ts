...
export class WorkersService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.logger.log(`Workers service initialized with ${this.queues.size} BullMQ queues`);
  }
  ...
  async getJobLogs(slug: string, jobId: string): Promise<{ logs: string[]; count: number }> {
    const queue = this.queues.get(slug);
    if (!queue) return { logs: [], count: 0 };
    return queue.getJobLogs(jobId);
  }

  async addJob(slug: string, data: Record<string, unknown>): Promise<{ jobId: string } | null> {
    const queue = this.queues.get(slug);
    if (!queue) return null;
  ...
  async cleanQueue(slug: string, status: 'completed' | 'failed', grace = 0): Promise<number> {
    const queue = this.queues.get(slug);
    if (!queue) return 0;
  ...
  async getKbBuilderStatus(): Promise<Record<string, unknown>> {
    try {
      ...
      return {
        data: {
          status: 'offline',
          message: 'KB Builder service is not running. Start it locally or deploy it.',
        },
      };
    }
  }
