...
describe("QuotaService", () => {
	...
	describe("checkAndIncrement", () => {
		it("should allow call and increment usage when within quota", async () => {
			...
			expect(tenantsRepo.incrementUsage).toHaveBeenCalledWith(
				"t1",
				"llmCallsUsed",
				1,
			);
		...
		it("should throw ForbiddenException when tenant not found", async () => {
			await expect(service.checkAndIncrement("missing")).rejects.toThrow(
				ForbiddenException,
			);
		});
		...
		it("should throw ForbiddenException when quota fully exceeded", async () => {
			...
			await expect(service.checkAndIncrement("t1")).rejects.toThrow(
				ForbiddenException,
			);
		...
		it("should throw ForbiddenException when remaining < requested calls", async () => {
			...
			await expect(service.checkAndIncrement("t1", 5)).rejects.toThrow(
				ForbiddenException,
			);
		});
	...
	describe("check", () => {
		...
		it("should throw ForbiddenException when tenant not found", async () => {
			await expect(service.check("missing")).rejects.toThrow(
				ForbiddenException,
			);
		});
