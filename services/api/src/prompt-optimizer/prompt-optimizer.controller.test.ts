...
describe('PromptOptimizerController', () => {
  ...
  const mockOptimizeResult = {
    optimizedPrompt: 'You are an expert data analyst. Analyze the following dataset...',
  ...
  beforeEach(async () => {
    ...
    controller = module.get<PromptOptimizerController>(PromptOptimizerController);
  });
  ...
  describe('analyze', () => {
    it('should return analysis result wrapped in data envelope', async () => {
      ...
      expect(mockService.analyzePrompt).toHaveBeenCalledWith('Tell me about AI', undefined);
    });

    it('should propagate service errors', async () => {
      mockService.analyzePrompt.mockRejectedValue(new Error('OpenAI unavailable'));

      await expect(controller.analyze({ prompt: 'test' })).rejects.toThrow('OpenAI unavailable');
    });
  });
  ...
  describe('optimize', () => {
    ...
    it('should propagate service errors', async () => {
      ...
      await expect(controller.optimize({ prompt: 'test', goal: 'test goal' })).rejects.toThrow(
        'Rate limited',
      );
    });
  });
