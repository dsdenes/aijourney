...
function mockFetch(summaries: unknown[] = [], articles: unknown[] = []) {
  globalThis.fetch = vi.fn().mockImplementation(async (url: string) => {
    ...
    if (urlStr.includes('/rag/query')) {
      // RAG query endpoint — return empty so it falls back to keyword search
      return {
        ok: true,
        json: async () => ({ data: { chunks: [], tokensUsed: 0 } }),
      };
    }
...
describe('ChatService', () => {
  ...
  describe('technicalSteps', () => {
    it('should include RAG provider step in response', async () => {
      ...
      const ragStep = result.technicalSteps!.find((s) => s.includes('RAG provider'));
    ...
    it('should include Gemini model name in technical steps', async () => {
      ...
      const modelStep = result.technicalSteps!.find((s) => s.includes('Gemini'));
    ...
    it('should include token usage breakdown in technical steps', async () => {
      ...
      const tokenStep = result.technicalSteps!.find((s) => s.toLowerCase().includes('tokens'));
    ...
    it('should include conversation history count in technical steps', async () => {
      ...
      const historyStep = result.technicalSteps!.find((s) => s.includes('previous messages'));
