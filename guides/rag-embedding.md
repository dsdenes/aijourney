...
- **Index-time**: embed _chunks_ (and store vectors + metadata in a vector index).
- **Query-time**: embed _the user query_ (or query variants), retrieve nearest chunk vectors, then feed retrieved text to the generator.

**Most common embedding failure mode**: you embed documents and queries with _incompatible conventions_ (wrong prefix/instruction, different normalization, different model/version), so “similar” content is no longer close in vector space.
