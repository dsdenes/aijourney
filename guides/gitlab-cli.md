Below is a deterministic, agent-oriented operating guide for the **GitLab CLI (`glab`)** (the official GitLab CLI). It assumes an agent is operating on a local workstation repo _or_ inside GitLab CI. ([GitLab Docs][1])
...
When _not_ operating in a checked-out repo, agent MUST provide `-R <GROUP/NAMESPACE/REPO>` on commands that support it (issues, MRs, CI, etc.). ([GitLab Docs][14])
