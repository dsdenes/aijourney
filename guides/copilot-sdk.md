...
export class AgentHost {
  ...
  constructor(private runs: RunRegistry, private audit: AuditSink) {}
  ...
  async runCoder(runId: RunId, prompt: string) {
    ...
    try {
      ...
      await this.audit.append({ runId, sessionId: session.sessionId, ts: Date.now(), error: String(e?.stack ?? e) });
