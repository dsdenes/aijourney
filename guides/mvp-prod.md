...
Work whose primary value is long-term reliability/scale/ops maturity, *not learning*.
...
MVP still needs *some* signal, but not a full observability program.
...
When triggers apply, agents MUST implement the minimum viable *production* controls for that trigger (Section 5), not “everything”.
