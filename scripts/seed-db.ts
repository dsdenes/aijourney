...
async function main() {
  ...
  await db.collection('steps').createIndex({ journeyId: 1, order: 1 }, { name: 'journeyId_order' });
  console.log('  ✓ steps');
  ...
  await db
    .collection('evidence')
    .createIndex({ kpiId: 1, submittedAt: -1 }, { name: 'kpiId_submittedAt_desc' });
  ...
  await db
    .collection('run_requests')
    .createIndex({ status: 1, createdAt: -1 }, { name: 'status_createdAt_desc' });
  ...
  await db
    .collection('articles')
    .createIndex({ status: 1, fetchedAt: -1 }, { name: 'status_fetchedAt_desc' });
  await db.collection('articles').createIndex({ url: 1 }, { unique: true, name: 'url_unique' });
  console.log('  ✓ articles');

  await db.collection('summaries').createIndex({ articleId: 1 }, { name: 'articleId' });
  console.log('  ✓ summaries');

  await db
    .collection('events')
    .createIndex({ userId: 1, timestamp: -1 }, { name: 'userId_timestamp_desc' });
  console.log('  ✓ events');

  // Multi-tenant collections
  await db.collection('tenants').createIndex({ slug: 1 }, { unique: true, name: 'slug_unique' });
  console.log('  ✓ tenants');
  ...
  await db.collection('users').createIndex({ tenantId: 1, email: 1 }, { name: 'tenantId_email' });
  await db
    .collection('users')
    .createIndex({ googleId: 1 }, { unique: true, sparse: true, name: 'googleId_unique' });
  await db
    .collection('journeys')
  ...
  await db
    .collection('article_recommendations')
    .createIndex({ userId: 1, status: 1 }, { name: 'userId_status' });
  await db.collection('article_recommendations').createIndex({ batchId: 1 }, { name: 'batchId' });
  await db
    .collection('article_recommendations')
    .createIndex({ tenantId: 1, createdAt: -1 }, { name: 'tenantId_createdAt_desc' });
  await db
    ...
    .createIndex({ tenantId: 1, createdAt: -1 }, { name: 'tenantId_createdAt_desc' });
  await db.collection('article_rec_batches').createIndex({ status: 1 }, { name: 'status' });
