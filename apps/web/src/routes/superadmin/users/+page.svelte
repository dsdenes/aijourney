        ...
        <tbody>
          {#each users as user}
            <tr class="border-b border-border/50 transition-colors hover:bg-surface-dark/50">
              ...
              <td class="px-4 py-3">
                <span class="rounded-full px-2.5 py-0.5 text-xs font-medium {user.orgRole === 'admin' ? 'bg-primary/20 text-primary' : 'bg-surface-dark text-text-muted'}">
                  {user.orgRole}
                </span>
              </td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3">
                {#if user.email.toLowerCase() === PROTECTED_SUPERADMIN_EMAIL}
                  <span class="text-xs text-text-muted italic">Protected</span>
                {:else}
                  <button
                    type="button"
